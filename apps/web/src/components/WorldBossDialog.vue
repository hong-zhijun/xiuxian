...
奖励：每关单独结算 —— 参与宗门各得自己 1.2 小时产出的 灵石/药材/矿石（伤害第 1/2/3 名额外 ×1.5/1.25/1.1），
      每往后一关奖励 +20%，第 5 关起不再增加；入账不超过资源容量，仓库满了多出的部分作废；具体数字点「奖励」查看；
    ...
    <div v-else-if="showRewards" class="boss-rules">
      <template v-if="panel?.rewardPreview">
        ...
        <ul class="boss-reward-notes">
          ...
          <li>每往后一关，资源奖励 +20%（第 5 关起不再增加）；入账不超过资源容量，溢出作废</li>
